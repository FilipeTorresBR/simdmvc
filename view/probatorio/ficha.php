<?php
$id = base64_decode($_GET['s']);
$ini = "ini". base64_decode($_GET['p']);
$fim =  "fim". base64_decode($_GET['p']);
$hoje = date('Y-m-d');
setlocale(LC_TIME, 'pt_BR', 'pt_BR.utf-8', 'pt_BR.utf-8', 'portuguese');
date_default_timezone_set('America/Belem');


foreach ($this->model->ficha($id) as $r): 

    if (strtotime($hoje) < strtotime($r->$fim)){
    ...
    $siape = $r->SIAPE;
    $cargo = $r->Cargo;
    $data_admissao = strftime('%d de %B de %Y', strtotime($r->data_posse));
    $data_formalizacao = strftime('%d de %B de %Y', strtotime($r->data_exercicio));
    $lotacao = $r->Lotacao;
    $data1 = strftime('%d de %B de %Y', strtotime($r->$ini));
    $ano = strftime('%Y', strtotime($r->$fim));
    $data2 = strftime('%d de %B de %Y', strtotime($r->$fim));
